...
- **隐私**: 密钥存 `~/.config/opencode/lensweaver.env`, 支持 `VISION_OFFLINE=1` 纯本地模式

## 架构

```
opencode (DS 主模型)                    LensWeaver 插件
        │                                      │
  需要看图 ──────────── 调用 vision 工具 ────────┤
        │                                      │
...
```
plugin/
├── lensweaver.ts            # 插件入口 (id: lensweaver), 注册 5 个工具
└── lensweaver-lib/
    ├── config.ts            # 配置: 环境变量 + lensweaver.env
...
```powershell
# 1. 克隆仓库
git clone https://github.com/liumingshuo77-cmyk/lensweaver
cd lensweaver
...
# 3. (可选) 配置视觉后端
notepad "$env:USERPROFILE\.config\opencode\lensweaver.env"
```
...
## 配置项 (`lensweaver.env` 或环境变量, 环境变量优先)
