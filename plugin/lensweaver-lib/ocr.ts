...
function handleLine(line: string) {
  ...
  else call.reject(new Error(`LensWeaver worker: ${msg.error ?? "unknown error"}`))
}
...
function startWorker(cfg: VisionConfig): ChildProcess {
  ...
  child.on("exit", (code) => {
    ...
    failPending(`LensWeaver worker exited unexpectedly (code ${code})`)
  })
  return child
}
...
async function rpc<T>(cfg: VisionConfig, type: string, payload: Record<string, unknown>, timeoutMs = 30000): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => {
      pending.delete(id)
      reject(new Error(`LensWeaver worker: ${type} timed out after ${timeoutMs}ms`))
    }, timeoutMs)
    ...
    child.stdin?.write(line, (err) => {
      if (err) {
        clearTimeout(timer)
        pending.delete(id)
        reject(new Error(`LensWeaver worker: failed to send ${type}: ${err.message}`))
      }
    })
  })
}
