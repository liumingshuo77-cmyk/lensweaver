...
async function requestCompletion(
  ...
  if (config.offline) {
    throw new VisionError(
      "offline",
      "vision-pro: offline mode is enabled (VISION_OFFLINE=1), cloud vision tasks are disabled. Use vision_ocr for local-only OCR.",
    )
  }
  if (!config.apiBaseUrl) {
    throw new VisionError("config", "vision-pro: VISION_API_BASE_URL is not set")
  }
  const headers: Record<string, string> = { "Content-Type": "application/json" }
  if (config.apiKey) headers.Authorization = `Bearer ${config.apiKey}`
  let res: Response
  try {
    ...
    if (err instanceof Error && err.name === "TimeoutError") {
      throw new VisionError("network", `vision-pro: request to ${model} timed out`, true)
    }
    throw new VisionError("network", `vision-pro: network error calling ${model}: ${String(err)}`, true)
  }
  if (res.status === 401 || res.status === 403) {
    throw new VisionError("auth", "vision-pro: API key rejected (401/403). Set VISION_API_KEY")
  }
...
export class CloudClient {
  ...
  async structured(call: StructuredCall): Promise<{ value: unknown; model: string; attempts: number }> {
    ...
    if (candidates.length === 0) {
      throw new VisionError(
        "model",
        "vision-pro: no vision model configured. Set VISION_MODELS or add vision-capable models to VISION_API_BASE_URL",
      )
    }
    ...
    for (const model of candidates) {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        try {
          ...
          throw new VisionError("parse", `vision-pro: model ${model} returned unparseable JSON after ${parseRetries} retries: ${parsed.error}`)
        } catch (err) {
    ...
    throw lastError ?? new VisionError("upstream", "vision-pro: cloud request failed")
  }
