...
export async function readImageFile(p: string, baseDir: string): Promise<ImageInput> {
  ...
  if (!mime) {
    throw new Error(`vision-pro: "${p}" is not a supported image (png/jpg/jpeg/webp/gif/bmp)`)
  }
...
export async function decodeBase64Image(payload: string): Promise<ImageInput> {
  ...
  try {
    ...
    throw new Error("vision-pro: imageBase64 is not valid base64")
  }
  if (bytes.length === 0) throw new Error("vision-pro: imageBase64 is empty")
  const mime = sniffMime(bytes)
  if (!mime) throw new Error("vision-pro: imageBase64 does not look like a supported image")
