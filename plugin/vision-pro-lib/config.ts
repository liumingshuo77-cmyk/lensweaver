...
const VISION_ENV_FILE = join(homedir(), ".config", "opencode", "vision-pro.env")
...
function defaultPython(): string {
  const candidates = [
    join(homedir(), ".config", "opencode", "vision-pro-venv", "Scripts", "python.exe"),
    join(homedir(), ".config", "opencode", "vision-pro-venv", "bin", "python"),
  ]
...
export function loadConfig(): VisionConfig {
  ...
  const models = (firstEnv("VISION_MODELS", "VISION_MODEL") ?? DEFAULT_MODELS)
    ...
    .map((m) => m.trim())
    .filter(Boolean)
  const cacheDir = firstEnv("VISION_CACHE_DIR") ?? join(homedir(), ".cache", "vision-pro")
  return {
    apiBaseUrl: base,
